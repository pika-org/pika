...
namespace boost
{
  namespace inspect
  {
   windows_macro_check::windows_macro_check() : m_files_with_errors(0)
   {
   ...
   void windows_macro_check::inspect(
      const string & library_name,
      const path & full_path,   // example: c:/foo/boost/filesystem/path.hpp
      const string & contents )     // contents of file to be inspected
    {
      if (contents.find( "pikainspect:" "nowindows_macros" ) != string::npos) return;

      boost::sregex_iterator cur(contents.begin(),
          contents.end(), windows_macro_regex), end;
      ...
      for( ; cur != end; ++cur /*, ++m_files_with_errors*/ )
      {
        ...
        if(!(*cur)[3].matched)
        {
          ...
          error( library_name, full_path,
            "Windows macro clash: " + std::string((*cur)[0].first, (*cur)[0].second-1),
            line_number );
        }
      }
      if(errors > 0) {

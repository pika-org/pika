...
class pika_list(gdb.Command):
  ...
  def __init__(self):
    super(pika_list, self).__init__("pika list", gdb.COMMAND_USER, gdb.COMPLETE_NONE, True)
...
class pika_list_threads(gdb.Command):
  ...
  '''
  def __init__(self):
    super(pika_list_threads, self).__init__("pika list threads", gdb.COMMAND_USER, gdb.COMPLETE_NONE, False)
  ...
  def invoke(self, arg, from_tty):
    ...
    runtime = gdb.lookup_global_symbol("pika::runtime::runtime_").value()["ptr_"].dereference()
...
class pika_select_thread(gdb.Command):
  ...
  def __init__(self):
    super(pika_select_thread, self).__init__("pika thread", gdb.COMMAND_USER, gdb.COMPLETE_NONE, False)
  ...
  def invoke(self, arg, from_tty):
    ...
    if len(argv) != 1:
      print("Error: You need to supply at least one argument. See help pika thread")
      return
...
class pika_continue(gdb.Command):
  ...
  def __init__(self):
    super(pika_continue, self).__init__("pika continue", gdb.COMMAND_USER, gdb.COMPLETE_NONE, False)
  ...
  def invoke(self, arg, from_tty):
    ...
    while True:
        ...
        if function and function.name == "pika::util::command_line_handling::handle_attach_debugger()":
          handle_attach = True
          break
        frame = frame.older()

...
#define PIKA_ITT_SYNC_RENAME(obj, name) itt_sync_rename(obj, name)
#define PIKA_ITT_SYNC_PREPARE(obj) itt_sync_prepare(obj)
#define PIKA_ITT_SYNC_CANCEL(obj) itt_sync_cancel(obj)
#define PIKA_ITT_SYNC_ACQUIRED(obj) itt_sync_acquired(obj)
#define PIKA_ITT_SYNC_RELEASING(obj) itt_sync_releasing(obj)
#define PIKA_ITT_SYNC_RELEASED(obj) itt_sync_released(obj)
#define PIKA_ITT_SYNC_DESTROY(obj) itt_sync_destroy(obj)

#define PIKA_ITT_STACK_CREATE(ctx) ctx = itt_stack_create()
#define PIKA_ITT_STACK_CALLEE_ENTER(ctx) itt_stack_enter(ctx)
#define PIKA_ITT_STACK_CALLEE_LEAVE(ctx) itt_stack_leave(ctx)
#define PIKA_ITT_STACK_DESTROY(ctx) itt_stack_destroy(ctx)

#define PIKA_ITT_FRAME_BEGIN(frame, id) itt_frame_begin(frame, id)
#define PIKA_ITT_FRAME_END(frame, id) itt_frame_end(frame, id)

#define PIKA_ITT_MARK_CREATE(mark, name) mark = itt_mark_create(name)
#define PIKA_ITT_MARK_OFF(mark) itt_mark_off(mark)
#define PIKA_ITT_MARK(mark, parameter) itt_mark(mark, parameter)

#define PIKA_ITT_THREAD_SET_NAME(name) itt_thread_set_name(name)
...
#define PIKA_ITT_STRING_HANDLE_CREATE(name) itt_string_handle_create(name)

#define PIKA_ITT_MAKE_ID(addr, extra) itt_make_id(addr, extra)
...
PIKA_EXPORT void itt_sync_releasing(void* addr) noexcept;
PIKA_EXPORT void itt_sync_released(void* addr) noexcept;
PIKA_EXPORT void itt_sync_destroy(void* addr) noexcept;

PIKA_EXPORT ___itt_caller* itt_stack_create() noexcept;
PIKA_EXPORT void itt_stack_enter(___itt_caller* ctx) noexcept;
...
PIKA_EXPORT void itt_mark(int mark, char const*) noexcept;
...
PIKA_EXPORT int itt_event_create(char const* name, int namelen) noexcept;
...
namespace pika { namespace util { namespace itt {
    ...
    {
        PIKA_EXPORT caller_context(stack_context& ctx);
        PIKA_EXPORT ~caller_context();
    ...
    struct domain
    {
        PIKA_NON_COPYABLE(domain);

        domain() = default;
        PIKA_EXPORT domain(char const*) noexcept;
    ...
    {
        PIKA_NON_COPYABLE(thread_domain);

        PIKA_EXPORT thread_domain() noexcept;
    };
    ...
    {
        PIKA_EXPORT undo_frame_context(frame_context& frame) noexcept;
        PIKA_EXPORT ~undo_frame_context();
    ...
    struct mark_context
    {
        PIKA_EXPORT mark_context(char const* name) noexcept;
        PIKA_EXPORT ~mark_context();
    ...
    {
        PIKA_EXPORT undo_mark_context(mark_context& mark) noexcept;
        PIKA_EXPORT ~undo_mark_context();
    ...
    struct string_handle
    {
        ...
        PIKA_EXPORT string_handle(char const* s) noexcept;
    ...
    {
        PIKA_EXPORT heap_internal_access() noexcept;
        PIKA_EXPORT ~heap_internal_access();
    };

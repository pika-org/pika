...
namespace pika { namespace parallel { inline namespace v1 { namespace detail {
    ...
    struct sequential_fill_t
      : pika::functional::detail::tag_fallback<sequential_fill_t>
    {
    private:
...
#else
    template <typename ExPolicy, typename Iter, typename Sent, typename T>
    PIKA_HOST_DEVICE PIKA_FORCEINLINE Iter sequential_fill(
    ...
    struct sequential_fill_n_t
      : pika::functional::detail::tag_fallback<sequential_fill_n_t>
    {
    private:
...
#else
    template <typename ExPolicy, typename Iter, typename T>
    PIKA_HOST_DEVICE PIKA_FORCEINLINE Iter sequential_fill_n(

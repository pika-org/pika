...
namespace pika::parallel::detail {
    ...
    sequential_unique(FwdIter first, Sent last, Pred&& pred, Proj&& proj)
    {
        ...
        while (++first != last)
        {
            if (!PIKA_INVOKE(pred, result_projected, PIKA_INVOKE(proj, *first)))
            {
                if (++result != first)
                {
    ...
    template <typename FwdIter, typename Sent, typename OutIter, typename Pred,
        typename Proj>
    unique_copy_result<FwdIter, OutIter> sequential_unique_copy(FwdIter first,
        Sent last, OutIter dest, Pred&& pred, Proj&& proj, std::true_type)
    {
        if (first == last)
        ...
        Sent last, OutIter dest, Pred&& pred, Proj&& proj, std::false_type)
    {
        ...
        using element_type = typename std::iterator_traits<InIter>::value_type;
    ...
    struct unique_copy : public algorithm<unique_copy<IterPair>, IterPair>
    {
            ...
            Pred&& pred, Proj&& proj)
        {
            ...
            *dest++ = *first;
            ...
            auto f4 = [last_iter, dest, flags](std::vector<std::size_t>&& items,

...
namespace pika::parallel::util::detail {
        ...
        ExPolicy&& policy, FwdIter first, std::size_t count, F&& f)
    {
        ...
        auto shape = get_bulk_iteration_shape(has_variable_chunk_size{},
            PIKA_FORWARD(ExPolicy, policy), inititems, f, first, count, 1);
        ...
        FwdIter first, std::size_t count, Stride stride, F&& f)
    {
        ...
        std::vector<pika::future<Result>> inititems;
        auto shape = get_bulk_iteration_shape_idx(has_variable_chunk_size{},
            PIKA_FORWARD(ExPolicy, policy), inititems, f, first, count, stride);
        ...
        std::vector<std::size_t> const& chunk_sizes, Data&& data, F&& f)
    {
        ...
        using tuple_type = typename std::tuple<typename data_type::value_type,
            FwdIter, std::size_t>;
    ...
    struct static_partitioner
    {
            ...
            std::size_t count, Stride stride, F1&& f1, F2&& f2)
        {
            ...
            try
            {
                workitems = partition_with_index<Result>(
            ...
            std::vector<std::size_t> const& chunk_sizes, Data&& data)
        {
            ...
            try
            {
                workitems = partition_with_data<Result>(
                    PIKA_FORWARD(ExPolicy_, policy), first, count, chunk_sizes,
                    PIKA_FORWARD(Data, data), PIKA_FORWARD(F1, f1));
    ...
    struct task_static_partitioner
    {
        ...
        template <typename ExPolicy_, typename FwdIter, typename F1,
            typename F2, typename Data>
        // requires is_container<Data>
        static pika::future<R> call_with_data(ExPolicy_&& policy, FwdIter first,
            std::size_t count, F1&& f1, F2&& f2,
            std::vector<std::size_t> const& chunk_sizes, Data&& data)
        {
            // inform parameter traits
            ...
            std::list<std::exception_ptr>&& errors, F&& f)
        {
            ...
            return pika::dataflow(
                [errors = PIKA_MOVE(errors),
                    ...
                    handle_exceptions::call(r, errors);
    ...
    struct partitioner
      : select_partitioner<std::decay_t<ExPolicy>, static_partitioner,

...
namespace pika::parallel::detail {
            ...
            std::size_t count, F1&& f1, F2&& f2)
        {
            ...
            try
            {
                std::tie(inititems, workitems) =
                    foreach_partition<Result>(PIKA_FORWARD(ExPolicy_, policy),
    ...
    private:
            ...
            std::list<std::exception_ptr>&& errors, F&& f, FwdIter last)
        {
            ...
            // exceptional future
            handle_exceptions::call(inititems, errors);
            ...
            std::list<std::exception_ptr>&& errors, F&& f, FwdIter last)
        {
            ...
            return pika::dataflow(
                [last, errors = PIKA_MOVE(errors),
                    ...
                    std::vector<pika::future<Result>> r2) mutable -> FwdIter {
                    PIKA_UNUSED(scoped_params);

                    handle_exceptions::call(r1, errors);
                    handle_exceptions::call(r2, errors);
                    return f(PIKA_MOVE(last));
                },
    ...
    struct foreach_partitioner
      : select_partitioner<std::decay_t<ExPolicy>, foreach_static_partitioner,

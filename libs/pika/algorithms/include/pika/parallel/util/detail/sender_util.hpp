...
namespace pika { namespace detail {
    ...
    auto then_with_bound_algorithm(Predecessor&& predecessor, ExPolicy&& policy)
    {
        ...
        if constexpr (pika::execution::detail::has_async_execution_policy_v<
                          ExPolicy>)
        {
            auto task_policy =
                PIKA_FORWARD(ExPolicy, policy)(pika::execution::task);
            return pika::execution::experimental::let_value(
                PIKA_FORWARD(Predecessor, predecessor),
                bound_algorithm<Tag, decltype(task_policy)>{
                    PIKA_MOVE(task_policy)});
        }
        ...
        {
            return pika::execution::experimental::then(
                PIKA_FORWARD(Predecessor, predecessor),
                bound_algorithm<Tag, ExPolicy>{PIKA_FORWARD(ExPolicy, policy)});
        }
    }
    ...
    struct tag_parallel_algorithm : pika::functional::detail::tag_fallback<Tag>
    {
        // clang-format off
        template <typename Predecessor, typename ExPolicy,
            PIKA_CONCEPT_REQUIRES_(
                std::conjunction_v<
                    pika::is_execution_policy<ExPolicy>,
                    std::negation<detail::is_bound_algorithm<Predecessor>>,
                    pika::execution::experimental::is_sender<
        ...
        {
            return pika::execution::experimental::detail::partial_algorithm<Tag,

...
namespace pika { namespace parallel { namespace util { namespace detail {
    ...
    {
        workitems.push_back(
            pika::make_ready_future(PIKA_FORWARD(F, f)(first, count)));
    }

    template <typename F, typename FwdIter>
    void add_ready_future(std::vector<pika::future<void>>& workitems, F&& f,
        FwdIter first, std::size_t count)
    {
        PIKA_FORWARD(F, f)(first, count);
        workitems.push_back(pika::make_ready_future());
    }

    template <typename F, typename FwdIter>
    void add_ready_future(std::vector<pika::shared_future<void>>& workitems,
        ...
        FwdIter& begin, std::size_t& count, Stride s)
    {
        ...
        return pika::util::make_iterator_range(shape_begin, shape_end);
    }
    ...
    template <typename F, typename FwdIter>
    void add_ready_future_idx(std::vector<pika::future<void>>& workitems, F&& f,
        FwdIter first, std::size_t base_idx, std::size_t count)
    {
    ...
    template <typename F, typename FwdIter>
    void add_ready_future_idx(std::vector<pika::shared_future<void>>& workitems,

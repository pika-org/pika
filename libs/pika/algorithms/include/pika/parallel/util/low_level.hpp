...
namespace pika { namespace parallel { namespace util {
    ...
    {
        (::new (static_cast<void*>(ptr)) Value(PIKA_FORWARD(Args, args)...));
    }
        ...
        typename std::iterator_traits<Iter>::value_type& val)
    {
        ...
        construct_object(&(*first), PIKA_MOVE(val));

        Iter it1 = first, it2 = first + 1;
        while (it2 != last)
        {
            // NOLINTNEXTLINE(bugprone-macro-repeated-side-effects)
            construct_object(&(*(it2++)), PIKA_MOVE(*(it1++)));
        }

        val = PIKA_MOVE(*(last - 1));
    }
    ...
    inline Value* uninit_move(Value* ptr, Iter first, Sent last)
    {
        ...
        {
            // NOLINTNEXTLINE(bugprone-macro-repeated-side-effects)
            ::new (static_cast<void*>(ptr++)) Value(PIKA_MOVE(*(first++)));
        }

...
namespace pika::parallel::detail {
    ...
    using range = pika::util::iterator_range<Iterator, Sentinel>;
        ...
        range<Iter1, Sent1> rng2, Compare cmp)
    {
        ...
        while (rx1.begin() != rx1.end())
        {
            ...
            {
                *rx1.begin() = PIKA_MOVE(*rbx.begin());
                rbx = range<Iter2, Sent2>(rbx.begin() + 1, rbx.end());
            }
            else
            {
                *rx1.begin() = PIKA_MOVE(*rx2.begin());
        ...
        {
            detail::init_move(rbuf, rng2);
        }
        else
        {
            detail::half_merge(rbuf, rx2, rbx, cmp);
        }
    }
}    // namespace pika::parallel::detail

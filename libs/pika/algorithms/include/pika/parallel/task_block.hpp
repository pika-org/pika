...
namespace pika { namespace parallel { inline namespace v2 {
    ...
    class task_canceled_exception : public pika::exception
    {
    public:
    ...
    ///
    template <typename ExPolicy = pika::execution::parallel_policy>
    class task_block
    {
    private:
        /// \cond NOINTERNAL
        ...
        {
            tasks_.add_exception(PIKA_MOVE(p));
        }
        /// \endcond
    ...
    template <typename ExPolicy, typename F,
        PIKA_CONCEPT_REQUIRES_(
            pika::is_async_execution_policy<std::decay_t<ExPolicy>>::value
        )>
    // clang-format on
    pika::future<void> define_task_block(ExPolicy&& policy, F&& f)
    {
        return pika::async(policy.executor(), detail::define_task_block,
            PIKA_FORWARD(ExPolicy, policy), PIKA_FORWARD(F, f));
    }
    ...
    {
        detail::define_task_block(pika::execution::par, PIKA_FORWARD(F, f));
    }
    ...
    define_task_block_restore_thread(ExPolicy&& policy, F&& f)
    {
        ...
        // By design we always return on the same (pika-) thread as we started

    ...
    OrgIter org_last, FwdIter first, FwdIter last, ValueType value)
{
    ...
    for (int i = 0; i < test_count; ++i)
    {
        ...
        auto start = high_resolution_clock::now();
        pika::remove(policy, first, last, value);
        dur += high_resolution_clock::now() - start;
    }

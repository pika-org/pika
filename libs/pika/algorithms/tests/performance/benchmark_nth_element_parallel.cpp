...
void function01(void)
{
    ...
    {
        B = A;
        pika::nth_element(::pika::execution::par, B.begin(), B.begin() + i,
            B.end(), compare_t());
    }
...
/// Accumulate all the times and compare the two algorithms with N threads
//----------------------------------------------------------------------------
...
int test_main(void)
{
    ...
    std::cout
        << "**********************************************************\n\n";
    std::cout << "        BENCHMARK_NTH_ELEMENT_PARALLEL (N Threads)\n";

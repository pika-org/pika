...
double run_remove_if_benchmark_pika(int test_count, ExPolicy policy,
    OrgIter org_first, OrgIter org_last, FwdIter first, FwdIter last, Pred pred)
{
    ...
    for (int i = 0; i < test_count; ++i)
    {
        ...
        auto start = high_resolution_clock::now();
        pika::remove_if(policy, first, last, pred);
        dur += high_resolution_clock::now() - start;
    }
    ...
    std::size_t random_range, IteratorTag, DataType)
{
    ...
    pika::generate(par, std::begin(v), std::end(v), random_fill(random_range));
    org_v = v;
...
int main(int argc, char* argv[])
{
    ...
    desc_commandline.add_options()("vector_size",
        ...
        pika::program_options::value<std::string>()->default_value("int"),

...
#endif
//
#define PIKA_REDUCE_BY_KEY_TEST_SIZE (1 << 18)
//
    ...
    const Op& op, const HelperOp& ho)
{
    ...
    values.reserve(PIKA_REDUCE_BY_KEY_TEST_SIZE);
    keys.reserve(PIKA_REDUCE_BY_KEY_TEST_SIZE);
    ...
    for (/* */; keysize < PIKA_REDUCE_BY_KEY_TEST_SIZE;)
    {
        do
        {
        ...
        //
        Tval sum = 0;
        for (int i = 0; i < numkeys && keysize < PIKA_REDUCE_BY_KEY_TEST_SIZE;
             ++i)
        {
    ...
    auto result =
        pika::reduce_by_key(std::forward<ExPolicy>(policy), keys.begin(),
            keys.end(), values.begin(), keys.begin(), values.begin(), op);
    ...
    if (is_equal)
    {
        if (benchmark)
        {
            ...
            pika::util::print_cdash_timing("ReduceByKeyTime", elapsed);
        }
    }
    else
    {
    ...
    const Op& op, const HelperOp& ho)
{
    ...
    pika::chrono::detail::high_resolution_timer t;
    // reduce_by_key, blocking when seq, par, par_vec
    auto result = pika::reduce_by_key(std::forward<ExPolicy>(policy),

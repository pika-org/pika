...
void test_max_element(IteratorTag)
{
    ...
    r = pika::max_element(iterator(std::begin(c)), iterator(std::end(c)));
    PIKA_TEST(r != end);
...
void test_max_element(ExPolicy policy, IteratorTag)
{
    ...
    iterator r = pika::max_element(policy, iterator(std::begin(c)),
...
void test_max_element_async(ExPolicy p, IteratorTag)
{
    ...
    pika::future<iterator> r = pika::max_element(
    ...
    r = pika::max_element(p, iterator(std::begin(c)), iterator(std::end(c)));
    rit = r.get();
    PIKA_TEST(rit != end);
...
void test_max_element_exception_async(ExPolicy p, IteratorTag)
{
...

    {
        ...
        try
        {
            pika::future<decorated_iterator> f = pika::max_element(p,
                decorated_iterator(
                    std::begin(c), []() { throw std::runtime_error("test"); }),
                decorated_iterator(std::end(c)), std::less<std::size_t>());

...
void test_replace_if(IteratorTag)
{
    ...
    pika::replace_if(iterator(std::begin(c)), iterator(std::end(c)),
...
void test_replace_if(ExPolicy policy, IteratorTag)
{
    ...
    pika::replace_if(policy, iterator(std::begin(c)), iterator(std::end(c)),
        equal_f(c[idx]), c[idx] + 1);
...
void test_replace_if_async(ExPolicy p, IteratorTag)
{
    ...
    pika::future<void> f = pika::replace_if(p, iterator(std::begin(c)),
...
void test_replace_if_exception_async(ExPolicy p, IteratorTag)
{
    ...
    try
    {
        pika::future<void> f = pika::replace_if(p,
            decorated_iterator(
                std::begin(c), []() { throw std::runtime_error("test"); }),
            decorated_iterator(std::end(c)), equal_f(42), std::size_t(43));

...
void test_for_loop_n(ExPolicy&& policy, IteratorTag)
{
    ...
    pika::for_loop_n(std::forward<ExPolicy>(policy), iterator(std::begin(c)),
...
void test_for_loop_n_idx(ExPolicy&& policy)
{
    ...
    pika::for_loop_n(std::forward<ExPolicy>(policy), 0, c.size(),
...
void test_for_loop_n_idx_async(ExPolicy&& p)
{
    ...
    auto f = pika::for_loop_n(std::forward<ExPolicy>(p), 0, c.size(),
        [&c](std::size_t i) { c[i] = 42; });
    f.wait();

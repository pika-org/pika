...
void test1()
{
    ...
    R[2] = RC;
    R[3] = RD;
    PIKA_TEST(RD.size() == 10);
    RX = full_merge4(RX, R, 4, comp);
    PIKA_TEST(RX.size() == 10);
    ...
    R[2] = RC;
    R[3] = RD;
    PIKA_TEST(RA.size() == 10 && RD.size() == 10);
    RX = full_merge4(RX, R, 4, comp);
    PIKA_TEST(RX.size() == 20);
    for (std::uint32_t i = 0; i < X.size(); ++i)
        PIKA_TEST(X[i] == i);
...
void test2()
{
    ...
    PIKA_TEST(RX.size() == 40);
    for (std::uint32_t i = 0; i < X.size(); ++i)
        PIKA_TEST(X[i].num == i / 4 && X[i].tail == i % 4);
}

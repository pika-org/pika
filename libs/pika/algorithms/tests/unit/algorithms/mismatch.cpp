...
void test_mismatch1(IteratorTag)
{
    ...
    {
        auto result = pika::mismatch(begin1, end1, std::begin(c2));

        // verify values
        PIKA_TEST_EQ(
            std::size_t(std::distance(begin1, result.first)), c1.size());
        PIKA_TEST_EQ(std::size_t(std::distance(std::begin(c2), result.second)),
            c2.size());
    }
...
void test_mismatch1(ExPolicy&& policy, IteratorTag)
{
    ...
    {
        auto result = pika::mismatch(policy, begin1, end1, std::begin(c2));
...
void test_mismatch1_async(ExPolicy&& p, IteratorTag)
{
    ...
    {
        auto f = pika::mismatch(p, begin1, end1, std::begin(c2));
        f.wait();
...
void test_mismatch_exception(IteratorTag)
{
    ...
    try
    {
        pika::mismatch(iterator(std::begin(c1)), iterator(std::end(c1)),
...
void test_mismatch_exception(ExPolicy&& policy, IteratorTag)
{
    ...
    try
    {
        pika::mismatch(policy, iterator(std::begin(c1)), iterator(std::end(c1)),
            std::begin(c2), [](std::size_t, std::size_t) {
                return throw std::runtime_error("test"), true;
            });
...
void test_mismatch_exception_async(ExPolicy&& p, IteratorTag)
{
    ...
    {
        auto f =
            pika::mismatch(p, iterator(std::begin(c1)), iterator(std::end(c1)),
                std::begin(c2), [](std::size_t, std::size_t) {
                    return throw std::runtime_error("test"), true;
                });

...
void test_uninitialized_move_n_async(ExPolicy p, IteratorTag)
{
    ...
    auto f = pika::uninitialized_move_n(
...
void test_uninitialized_move_n()
{
    ...
    test_uninitialized_move_n(pika::execution::seq, IteratorTag());
    test_uninitialized_move_n(pika::execution::par, IteratorTag());
    test_uninitialized_move_n(pika::execution::par_unseq, IteratorTag());
...
void test_uninitialized_move_n_exception(ExPolicy policy, IteratorTag)
{
    ...
    try
    {
        pika::uninitialized_move_n(policy,
...
void test_uninitialized_move_n_exception_async(ExPolicy p, IteratorTag)
{
    ...
    try
    {
        auto f = pika::uninitialized_move_n(p,
            decorated_iterator(std::begin(c),
                [&throw_after]() {
                    if (throw_after-- == 0)
                        throw std::runtime_error("test");

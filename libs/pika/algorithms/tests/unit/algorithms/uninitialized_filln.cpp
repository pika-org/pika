...
void test_uninitialized_fill_n(IteratorTag)
{
    ...
    pika::uninitialized_fill_n(iterator(std::begin(c)), c.size(), 10);
...
void test_uninitialized_fill_n(ExPolicy policy, IteratorTag)
{
    ...
    pika::uninitialized_fill_n(policy, iterator(std::begin(c)), c.size(), 10);
...
void test_uninitialized_fill_n_async(ExPolicy p, IteratorTag)
{
    ...
    pika::future<void> f =
        pika::uninitialized_fill_n(p, iterator(std::begin(c)), c.size(), 10);
    f.wait();
...
void test_uninitialized_fill_n_exception(ExPolicy policy, IteratorTag)
{
    ...
    try
    {
        pika::uninitialized_fill_n(policy,
...
void test_uninitialized_fill_n_exception_async(ExPolicy p, IteratorTag)
{
    ...
    try
    {
        pika::future<void> f = pika::uninitialized_fill_n(p,
            decorated_iterator(std::begin(c),
                [&throw_after]() {
                    if (throw_after-- == 0)
                        throw std::runtime_error("test");

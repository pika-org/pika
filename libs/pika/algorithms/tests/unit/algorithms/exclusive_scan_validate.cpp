...
#endif
    b.resize(a.size());
    pika::exclusive_scan(p, a.begin(), a.end(), b.begin(), INITIAL_VAL,
...
void exclusive_scan_validate()
{
    ...
    test_exclusive_scan_validate(pika::execution::seq, a, b);

    DEBUG_OUT("\nValidating separate arrays parallel");
    test_exclusive_scan_validate(pika::execution::par, a, b);

    // test scan algorithms using same array for input and output
    DEBUG_OUT("\nValidating in_place arrays sequential ");
    test_exclusive_scan_validate(pika::execution::seq, a, a);

    DEBUG_OUT("\nValidating in_place arrays parallel ");
    test_exclusive_scan_validate(pika::execution::par, a, a);
}

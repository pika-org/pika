...
void test_destroy(IteratorTag)
{
    ...
    pika::ranges::destroy(iterator(p), iterator(p + data_size));
...
void test_destroy_exception(IteratorTag)
{
    ...
    try
    {
        pika::ranges::destroy(decorated_iterator(p,
                                 [&throw_after]() {
                                     if (throw_after-- == 0)
...
void test_destroy_exception_async(ExPolicy&& policy, IteratorTag)
{
    ...
    try
    {
        auto f = pika::ranges::destroy(policy,
            decorated_iterator(p,
                [&throw_after]() {
                    if (throw_after-- == 0)
                        throw std::runtime_error("test");

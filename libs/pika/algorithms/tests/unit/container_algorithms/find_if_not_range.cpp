...
void test_find_if_not(IteratorTag)
{
    ...
    iterator index =
        pika::ranges::find_if_not(iterator(std::begin(c)), iterator(std::end(c)),
...
void test_find_if_not(ExPolicy&& policy, IteratorTag)
{
    ...
    iterator index = pika::ranges::find_if_not(policy, iterator(std::begin(c)),
        iterator(std::end(c)),
        [](std::size_t v) { return v != std::size_t(1); });
...
void test_find_if_not_async(ExPolicy&& p, IteratorTag)
{
    ...
    pika::future<iterator> f = pika::ranges::find_if_not(p,

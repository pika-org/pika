...
void test_partitioned1(ExPolicy policy, IteratorTag)
{
    ...
    bool parted = pika::ranges::is_partitioned(policy, iterator(std::begin(c)),
...
void test_partitioned1_async(ExPolicy p, IteratorTag)
{
    ...
    pika::future<bool> f =
        pika::ranges::is_partitioned(p, iterator(std::begin(c)),
            iterator(std::end(c)), [](std::size_t n) { return n % 2 == 0; });
    f.wait();
...
void test_partitioned1(ExPolicy policy)
{
    ...
    bool parted = pika::ranges::is_partitioned(
...
void test_partitioned1_async(ExPolicy p)
{
    ...
    pika::future<bool> f = pika::ranges::is_partitioned(
...
void test_partitioned2(ExPolicy policy, IteratorTag)
{
    ...
    bool parted_odd = pika::ranges::is_partitioned(policy,
        iterator(std::begin(c_odd)), iterator(std::end(c_odd)),
        [](std::size_t n) { return n % 2 == 0; });
    bool parted_even = pika::ranges::is_partitioned(policy,
...
void test_partitioned2_async(ExPolicy p, IteratorTag)
{
    ...
    pika::future<bool> f_odd = pika::ranges::is_partitioned(p,
        iterator(std::begin(c_odd)), iterator(std::end(c_odd)),
        [](std::size_t n) { return n % 2 == 0; });
    pika::future<bool> f_even = pika::ranges::is_partitioned(p,
        iterator(std::begin(c_even)), iterator(std::end(c_even)),
        [](std::size_t n) { return n % 2 == 0; });
...
void test_partitioned2(ExPolicy policy)
{
    ...
    bool parted_odd = pika::ranges::is_partitioned(
        policy, c_odd, [](std::size_t n) { return n % 2 == 0; });
    bool parted_even = pika::ranges::is_partitioned(
...
void test_partitioned2_async(ExPolicy p)
{
    ...
    pika::future<bool> f_odd = pika::ranges::is_partitioned(
        p, c_odd, [](std::size_t n) { return n % 2 == 0; });
    pika::future<bool> f_even = pika::ranges::is_partitioned(
...
void test_partitioned3(ExPolicy policy, IteratorTag)
{
    ...
    bool parted1 = pika::ranges::is_partitioned(policy,
        iterator(std::begin(c_beg)), iterator(std::end(c_beg)),
        [](std::size_t n) { return n % 2 == 0; });
    bool parted2 = pika::ranges::is_partitioned(policy,
...
void test_partitioned3_async(ExPolicy p, IteratorTag)
{
    ...
    pika::future<bool> f_beg = pika::ranges::is_partitioned(p,
        iterator(std::begin(c_beg)), iterator(std::end(c_beg)),
        [](std::size_t n) { return n % 2 == 0; });
    pika::future<bool> f_end = pika::ranges::is_partitioned(p,
        iterator(std::begin(c_end)), iterator(std::end(c_end)),
        [](std::size_t n) { return n % 2 == 0; });
...
void test_partitioned3(ExPolicy policy)
{
    ...
    bool parted1 = pika::ranges::is_partitioned(
        policy, c_beg, [](std::size_t n) { return n % 2 == 0; });
    bool parted2 = pika::ranges::is_partitioned(
...
void test_partitioned3_async(ExPolicy p)
{
    ...
    pika::future<bool> f_beg = pika::ranges::is_partitioned(
        p, c_beg, [](std::size_t n) { return n % 2 == 0; });
    pika::future<bool> f_end = pika::ranges::is_partitioned(
...
void test_partitioned_exception(ExPolicy policy, IteratorTag)
{
    ...
    try
    {
        pika::ranges::is_partitioned(policy,
            decorated_iterator(
...
void test_partitioned_async_exception(ExPolicy p, IteratorTag)
{
    ...
    try
    {
        pika::future<bool> f = pika::ranges::is_partitioned(p,
            decorated_iterator(
                std::begin(c), []() { throw std::runtime_error("test"); }),
            decorated_iterator(
                std::end(c), []() { throw std::runtime_error("test"); }),

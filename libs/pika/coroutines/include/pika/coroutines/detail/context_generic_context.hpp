...
void __splitstack_block_signals_context(
    void* [PIKA_COROUTINES_SEGMENTS], int* new_value, int* old_value);
void __splitstack_getcontext(void* [PIKA_COROUTINES_SEGMENTS]);
...
namespace pika { namespace threads { namespace coroutines {
    ...
    namespace detail { namespace generic_context {
        ...
        struct stack_allocator
        {
            ...
            static std::size_t maximum_stacksize()
            {
                PIKA_ASSERT_MSG(false, "segmented stack is unbound");
                return 0;
            }
        ...
        {
        public:
            PIKA_NON_COPYABLE(fcontext_context_impl);

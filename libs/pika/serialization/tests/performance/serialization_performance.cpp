...
#include <pika/serialization/detail/preprocess_container.hpp>
...
namespace pika_test {
    ...
    {
        {
            pika::serialization::detail::preprocess_container p;
            pika::serialization::output_archive archiver(p);
            archiver << record;
            data.resize(p.size());
        }
        pika::serialization::output_archive archiver(data);
        archiver << record;
    }

    void from_string(Record& record, std::string const& data)
    {
        pika::serialization::input_archive archiver(data);
        archiver >> record;
    }
}    // namespace pika_test

void pika_serialization_test(std::size_t iterations)
{
    using namespace pika_test;
    ...
    std::cout << "pika: time    = " << duration << " milliseconds" << std::endl
              << std::endl;
}
...
int main(int argc, char** argv)
{
    ...
    try
    {
        iterations = pika::util::from_string<size_t>(argv[1]);
    }
    ...
    pika_serialization_test(iterations);
}

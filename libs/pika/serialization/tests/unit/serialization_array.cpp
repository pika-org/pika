...
#include <pika/serialization/multi_array.hpp>
...
{
    {
        ...
        pika::serialization::output_archive oarchive(
            buffer, pika::serialization::disable_data_chunking);
        std::vector<T> os;
        for (T c = minval; c < maxval; ++c)
        {
            os.push_back(c);
        }
        oarchive << pika::serialization::make_array(&os[0], os.size());

        pika::serialization::input_archive iarchive(buffer);
        std::vector<T> is;
        is.resize(os.size());
        iarchive >> pika::serialization::make_array(&is[0], is.size());
        PIKA_TEST_EQ(os.size(), is.size());
        for (std::size_t i = 0; i < os.size(); ++i)
        {
            PIKA_TEST_EQ(os[i], is[i]);
        }
    }
    {
        std::vector<char> buffer;
...
{
    {
        ...
        {
            PIKA_TEST_EQ(oarray[i], iarray[i]);
        }
    }
    {
        std::vector<char> buffer;
        ...
        for (std::size_t i = 0; i < oarray.size(); ++i)
        {
            PIKA_TEST_EQ(oarray[i].t_, iarray[i].t_);
        }
    }
}
    ...
    for (std::size_t i = 0; i < 3; ++i)
        for (std::size_t j = 0; j < 4; ++j)
            for (std::size_t k = 0; k < 2; ++k)
                PIKA_TEST_EQ(oarray[i][j][k], iarray[i][j][k]);
}
...
void test_array_of_vectors()
{
    ...
    for (std::size_t i = 0; i < N; ++i)
    {
        PIKA_TEST_EQ(oarray[i].size(), iarray[i].size());

        for (std::size_t j = 0; j < i; ++j)
        {
            PIKA_TEST_EQ(oarray[i][j], iarray[i][j]);
        }
    }
}

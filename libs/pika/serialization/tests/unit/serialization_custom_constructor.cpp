...
A* a_factory(pika::serialization::input_archive& ar)
{
    A* a = new A(123);
    ar >> *a;
    return a;
...
PIKA_SERIALIZATION_WITH_CUSTOM_CONSTRUCTOR(A, a_factory)
...
B* b_factory(pika::serialization::input_archive& ar)
{
    double b;
    ar& b;
...
PIKA_SERIALIZATION_REGISTER_CLASS(B)
PIKA_SERIALIZATION_WITH_CUSTOM_CONSTRUCTOR(B, b_factory)

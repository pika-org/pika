...
#include <pika/serialization/input_container.hpp>
...
namespace pika { namespace serialization {
    ...
    struct input_archive : basic_archive<input_archive>
    {
                ...
                buffer, chunks, inbound_data_size))
        {
            ...
            load(endianness);
            if (endianness)
                this->base_type::flags_ = pika::serialization::endian_big;
            ...
            T& t)
        {
            using use_optimized = std::integral_constant<bool,
                pika::traits::is_bitwise_serializable_v<T> ||
                    !pika::traits::is_not_bitwise_serializable_v<T>>;
        ...
        void load(bool& b)
        {
            ...
            PIKA_ASSERT(0 == static_cast<int>(b) || 1 == static_cast<int>(b));
        }
    ...
    private:
        ...
        void load_bitwise(T& t, std::false_type)
        {
            load_nonintrusively_polymorphic(
                t, pika::traits::is_nonintrusive_polymorphic<T>());
        }

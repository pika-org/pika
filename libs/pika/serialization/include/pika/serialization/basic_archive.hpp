...
namespace pika { namespace serialization {
    ...
    void PIKA_FORCEINLINE reverse_bytes(std::size_t size, char* address)
    {
    ...
    public:
        ...
        void invoke(T& t)
        {
#if !defined(PIKA_SERIALIZATION_HAVE_ALLOW_RAW_POINTER_SERIALIZATION)
            static_assert(!std::is_pointer_v<T>,
                "pika does not support serialization of raw pointers. "
        ...
        {
            return (flags_ & pika::serialization::endian_big) ? true : false;
        }

        bool endian_little() const
        {
            return (flags_ & pika::serialization::endian_little) ? true : false;
        }

        bool disable_array_optimization() const
        {
            return (flags_ & pika::serialization::disable_array_optimization) ?
                true :
                false;
        }

...
namespace pika { namespace serialization {
    ...
    // make_array function
    template <class T>
    PIKA_FORCEINLINE array<T> make_array(T* begin, std::size_t size)
    {
    ...
    {
        // clang-format off
        ar & pika::serialization::make_array(a.begin(), a.size());
        // clang-format on
    }
#endif
    ...
    {
        // clang-format off
        ar & pika::serialization::make_array(a.data(), a.size());
        // clang-format on
    }
    ...
    template <typename T>
    PIKA_FORCEINLINE output_archive& operator<<(output_archive& ar, array<T> t)
    {
        ar.invoke(t);
        return ar;
    }
    ...
    template <typename T>
    PIKA_FORCEINLINE input_archive& operator>>(input_archive& ar, array<T> t)
    {
        ar.invoke(t);
        return ar;
    }
    ...
    PIKA_FORCEINLINE output_archive& operator<<(output_archive& ar, T (&t)[N])
    {
    ...
    template <typename T, std::size_t N>
    PIKA_FORCEINLINE input_archive& operator>>(input_archive& ar, T (&t)[N])
    {
        array<T> array = make_array(t, N);
        ar.invoke(array);
        return ar;

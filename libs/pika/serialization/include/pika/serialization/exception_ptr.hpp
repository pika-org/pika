...
namespace pika { namespace util {
    enum exception_type
    {
        ...
        // pika::exception
        pika_exception = 11,
        pika_thread_interrupted_exception = 12,
...
namespace pika { namespace serialization {
    namespace detail {
        using save_custom_exception_handler_type =
            std::function<void(pika::serialization::output_archive&,
                std::exception_ptr const&, unsigned int)>;
        using load_custom_exception_handler_type =
            std::function<void(pika::serialization::input_archive&,
    ...
    PIKA_SERIALIZATION_SPLIT_FREE(std::exception_ptr)

...
@code
#include <pika/logging/format/formatter/named.hpp>
@endcode
...
@code
#define L_ PIKA_LOG_USE_LOG_IF_FILTER(g_l(), g_log_filter()->is_enabled() )
...
*/
    ...
    {
        PIKA_NON_COPYABLE(named_formatters);
        ...
        void add(std::string const& name, ptr_type p)
        {
            ...
            if (iter != formatters.end())
                ...
                formatters.push_back(named<ptr_type>{name, PIKA_MOVE(p)});
            compute_write_steps();
        }
    ...
    private:
        ...
        PIKA_EXPORT void compute_write_steps();
...
*/
    ...
    {
        PIKA_NON_COPYABLE(named_destinations);
...
*/
    ...
    */
        ...
        void operator()(message const& msg) const
        {
            ...
            message formatted(PIKA_MOVE(out));
    ...
    */
        ...
        {
            m_format.add(name, Formatter::make(PIKA_FORWARD(Args, args)...));
        }
    ...
    */
    ...
    private:
        PIKA_EXPORT void configure_formatter(std::string const& format);
        PIKA_EXPORT void configure_destination(std::string const& format);

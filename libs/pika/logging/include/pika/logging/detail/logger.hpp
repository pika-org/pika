...
namespace pika { namespace util { namespace logging {
    ...
    @code
    PIKA_DECLARE_LOG_FILTER(g_log_filter, filter::no_ts )
    PIKA_DECLARE_LOG(g_l, logger)
    ...
    */
    class logger
    {
        PIKA_NON_COPYABLE(logger);

        struct gather_holder : message
        {    //-V690
            PIKA_NON_COPYABLE(gather_holder);
    ...
    public:
        ...
        void write(message msg)
        {
            if (m_is_caching_off)
                ...
                m_cache.push_back(PIKA_MOVE(msg));
        }

...
namespace pika { namespace cuda { namespace experimental {
    ...
    namespace detail {
                    ...
                    [fdp = pika::memory::intrusive_ptr<future_data>(this)](
                        ...
                        else
                        {
                            ...
                            fdp->set_exception(std::make_exception_ptr(
                                cuda_exception(std::move(msg), status)));
                        }
                    },
                    stream);
            }
          ...
          : lcos::detail::future_data_allocator<void, Allocator>
        {
            ...
            // this is called from the nvidia backend on a non-pika thread
                ...
                cudaStream_t, cudaError_t error, void* user_data)
            {
                ...
                if (error != cudaSuccess)
                {
                    this_->set_exception(PIKA_GET_EXCEPTION(kernel_error,
                ...
                this_->set_data(pika::util::unused);
        ...
        // specified mode
        template <typename Allocator, typename Mode>
        pika::future<void> get_future(Allocator const& a, cudaStream_t stream)
        {
        ...
        // main API call to get a future from a stream using allocator
        template <typename Allocator>
        pika::future<void> get_future_with_event(
            Allocator const& a, cudaStream_t stream)
        {
        ...
        PIKA_EXPORT pika::future<void> get_future_with_event(cudaStream_t);
    }    // namespace detail

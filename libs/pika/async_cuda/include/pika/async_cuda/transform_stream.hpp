...
namespace pika { namespace cuda { namespace experimental {
    namespace detail {
        ...
        void set_value_immediate_void(cudaStream_t stream, R&& r, Ts&&... ts)
        {
            ...
            extend_argument_lifetimes(stream, PIKA_FORWARD(Ts, ts)...);
        }
                ...
                pika::detail::try_catch_exception_ptr(
                    [&]() {
                        ...
                        else
                        {
                            ...
                            auto t =
                                PIKA_INVOKE(f, PIKA_FORWARD(Ts, ts)..., stream);
        ...
        struct transform_stream_sender
        {
            ...
            {
                using result_type = typename pika::util::invoke_result<F, Ts...,
                    cudaStream_t>::type;
                using type =
                ...
                transform_stream_sender&& s, R&& r)
            {
                return pika::execution::experimental::connect(PIKA_MOVE(s.s),
                    transform_stream_receiver<R, F>{
                        PIKA_FORWARD(R, r), PIKA_MOVE(s.f), s.stream});
            }
        };
    ...
    // pika::execution::experimental::then. It has different semantics:
    ...
    inline constexpr struct transform_stream_t final
      : pika::functional::detail::tag_fallback<transform_stream_t>
    {
    private:
        template <typename S, typename F,
            ...
            transform_stream_t, S&& s, F&& f, cudaStream_t stream = {})
        {
            return detail::transform_stream_sender<S, F>{
                PIKA_FORWARD(S, s), PIKA_FORWARD(F, f), stream};
        }

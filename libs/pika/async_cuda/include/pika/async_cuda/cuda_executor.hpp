...
namespace pika { namespace cuda { namespace experimental {
    ...
    {
        using future_type = pika::future<void>;
    ...
    protected:
        ...
        std::shared_ptr<pika::cuda::experimental::target> target_;
    };
    ...
    struct cuda_executor : cuda_executor_base
    {
        ...
        {
            return apply(PIKA_FORWARD(F, f), PIKA_FORWARD(Ts, ts)...);
        }
        ...
        {
            return async(PIKA_FORWARD(F, f), PIKA_FORWARD(Ts, ts)...);
        }
    ...
    protected:
        ...
        void apply(R (*cuda_function)(Params...), Args&&... args)
        {
            ...
            helper(cuda_function, PIKA_FORWARD(Args, args)..., stream_);
        }
        ...
        template <typename R, typename... Params, typename... Args>
        pika::future<void> async(R (*cuda_kernel)(Params...), Args&&... args)
        {
            return pika::detail::try_catch_exception_ptr(
                [&]() {
                    ...
                    // insert the stream handle in the arg list and call the cuda function
                    detail::dispatch_helper<R, Params...> helper{};
                    helper(cuda_kernel, PIKA_FORWARD(Args, args)..., stream_);
                    return get_future();
                },
...
namespace pika { namespace parallel { namespace execution {
    ...
    /// \cond NOINTERNAL
    template <>
    struct is_one_way_executor<pika::cuda::experimental::cuda_executor>
      : std::true_type
    {
    ...
    template <>
    struct is_two_way_executor<pika::cuda::experimental::cuda_executor>
      : std::true_type
    {

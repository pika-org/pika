...
static void test_mixed_container_remap()
{
    // Traits
    {
        using pika::util::detail::container_remapping::has_push_back;
        PIKA_TEST_EQ((has_push_back<std::vector<int>, int>::value), true);
        PIKA_TEST_EQ((has_push_back<int, int>::value), false);
    }
    ...
    // Rebind
    {
        ...
        // Rebinds the values
        {
            ...
            PIKA_TEST(dest == (decltype(dest){0, 1, 2}));
        }
...
static void test_mixed_fall_through()
{
    ...
    traverse_pack(my_int_mapper{}, int(0),
        std::vector<std::vector<float>>{{1.f, 2.f}}, pika::make_tuple(1.f, 2.f));
...
static void test_strategic_traverse()
{
    ...
    // Remapping works with move-only objects
    {
        ...
        PIKA_TEST_EQ(*get<0>(res), 2U);
        PIKA_TEST_EQ(*get<1>(res), 3U);
        PIKA_TEST_EQ(*get<2>(res), 4U);
    }
    ...
    // as reference, while returning those as reference.
    {
        ...
        *ptr1 = 1;
        *ptr2 = 2;
        PIKA_TEST_EQ(*get<0>(ref), 1);
        PIKA_TEST_EQ(*get<1>(ref), 2);
    }
}
...
static void test_strategic_tuple_like_traverse()
{
    ...
    // Fixed size homogeneous container
    {
        ...
        PIKA_TEST(res == (std::array<float, 3>{{1.f, 1.f, 1.f}}));
    }
...
{
    template <typename T>
    auto operator()(T arg) -> decltype(pika::util::spread_this(arg, arg))
    {
        return pika::util::spread_this(arg, arg);
    }
};

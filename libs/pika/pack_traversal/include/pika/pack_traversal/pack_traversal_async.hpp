...
namespace pika { namespace util {
    ...
    ///                  for referencing it from a `pika::intrusive_ptr`.
    ...
    ///
    /// \returns         A pika::intrusive_ptr that references an instance of
    ...
    auto traverse_pack_async(Visitor&& visitor, T&&... pack)
        -> decltype(detail::apply_pack_transform_async(
            PIKA_FORWARD(Visitor, visitor), PIKA_FORWARD(T, pack)...))
    {

...
namespace pika { namespace util { namespace detail {
    ...
    namespace spreading {
        ...
        constexpr auto tupelize_or_void(T&&... args)
            -> decltype(voidify_empty_tuple(tupelize(PIKA_FORWARD(T, args)...)))
        {
            return voidify_empty_tuple(tupelize(PIKA_FORWARD(T, args)...));
        }
    ...
    namespace container_remapping {
        ...
        public:
            container_accessor(C&& container)
              : container_(PIKA_MOVE(container))
            {
            }
        ...
        container_accessor<T> container_accessor_of(T&& container)
        {
            ...
            return container_accessor<T>(PIKA_FORWARD(T, container));
        }
    ...
    class mapping_helper : protected mapping_strategy_base<Strategy>
    {
                ...
                PIKA_FORWARD(T, element)))
        {
            ...
            return invoke_mapper(PIKA_FORWARD(T, element));
        }
        ...
        auto traverse(Strategy, T&& element)
            -> decltype(std::declval<mapping_helper>().match(
                std::declval<container_category_of_t<
                    typename pika::util::decay_unwrap<T>::type>>(),
                std::declval<T>()));
    ...
    public:
        ...
        {
            try_traverse(strategy_traverse_tag{}, PIKA_FORWARD(T, element));
        }

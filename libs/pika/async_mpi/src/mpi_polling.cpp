...
namespace pika::mpi::experimental {
    ...
    namespace detail {
        ...
        using mpi_callback_queue_tuple =
        ...
        pika::threads::detail::polling_status poll()
        {
            ...
            if (!lk.owns_lock())
            {
                if constexpr (mpi_debug.is_enabled())
                {
                    ...
                    static auto poll_deb =
                        mpi_debug.make_timer(1, str<>("Poll - lock failed"));
            ...
            {
                mpi_debug.debug(str<>("Polling loop"), detail::mpi_data_,
                ...
                for (int i = 0; i < outcount; ++i)
                {
                    ...
                    if constexpr (mpi_debug.is_enabled())
                    {
                        mpi_debug.debug(str<>("CB invoke"), detail::mpi_data_,
                            "request",
                            hex<8>(detail::mpi_data_.request_vector_[index]),
                            "throttling", stream);
                    }
                    ...
                    if (stream.in_stream_ < stream.limit_)
                    {
                        std::unique_lock lk(stream.throttling_mtx_);
                        mpi_debug.debug(
                            str<>("throttling"), "notify_one", stream);

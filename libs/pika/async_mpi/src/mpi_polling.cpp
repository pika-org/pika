...
namespace pika::mpi::experimental {
    ...
    namespace detail {
        ...
        static std::size_t task_completion_mode_ =
            get_completion_mode_default();

        static std::string default_pool_name = "pika:mpi-pool";
        ...
        void wait_for_throttling_impl(mpi_stream& stream)
        {
            ...
            [[maybe_unused]] auto scp =
                mpi_debug<5>.scope("throttling", "wait", stream);
        ...
        std::uint32_t get_env_value(const char* s, std::uint32_t def)
        {
            ...
            if (env)
            {
                ...
                {
                    val = def;
                    std::cerr << str<>("get_env_value") << " invalid " << s
                              << " " << dec<4>(def) << std::endl;
                    PIKA_DP(mpi_debug<3>,
                        error(
                            str<>("get_env_value"), "invalid", s, dec<4>(def)));
                }
                PIKA_DP(mpi_debug<3>,
                    debug(str<>("get_env_value"), s, dec<4>(def)));
            }
            return val;
        }
        ...
        void add_to_request_callback_queue(request_callback&& req_callback)
        {
            ...
            if constexpr (mpi_debug<5>.is_enabled())
            {
                ...
                PIKA_DP(mpi_debug<5>,
                    debug(str<>("CB queued"), mpi_data_, "request",
            ...
            MPI_Request request, stream_type s)
        {
            ...
            if (flag)
            {
                using namespace pika::debug::detail;
                PIKA_DP(mpi_debug<5>,
                    debug(str<>("eager poll"), "success", "stream",
        ...
        pika::threads::detail::polling_status poll()
        {
            ...
            // start a scope block where the polling lock is held
            {
                ...
                if (!lk.owns_lock())
                {
                    if constexpr (mpi_debug<5>.is_enabled())
                    {
                        ...
                        static auto poll_deb = mpi_debug<5>.make_timer(
                            1, str<>("Poll - lock failed"));
                        PIKA_DP(
                            mpi_debug<5>, timed(poll_deb, detail::mpi_data_));
                    }
                    return polling_status::idle;
                }
                ...
                if constexpr (mpi_debug<5>.is_enabled())
                {
                    // for debugging, create a timer : debug info every N seconds
                    static auto poll_deb = mpi_debug<5>.make_timer(
                        1, str<>("Poll - lock success"));
                    PIKA_DP(mpi_debug<5>, timed(poll_deb, detail::mpi_data_));
                }
                ...
                if (detail::mpi_data_.max_polling_requests > 1)
                {
                    ...
                    {
                        PIKA_DP(mpi_debug<5>,
                            debug(str<>("Polling loop"), detail::mpi_data_,
                                "outcount", dec<3>(outcount)));
            ...
            if constexpr (mpi_debug<5>.is_enabled())
            {
                static auto poll_deb =
                    mpi_debug<5>.make_timer(1, str<>("Poll - success"));
                PIKA_DP(mpi_debug<5>, timed(poll_deb, detail::mpi_data_));
            }
            ...
            for (const auto& c : callbacks_)
            {
                ...
                {
                    PIKA_DP(mpi_debug<5>,
                        debug(str<>("CB invoke"), detail::mpi_data_, "request",
                            hex<8>(std::get<MPI_Request>(c)), "throttling",
                            stream));
                }
                ...
                if (stream.in_stream_ < stream.limit_)
                {
                    std::unique_lock lk(stream.throttling_mtx_);
                    PIKA_DP(mpi_debug<5>,
                        debug(str<>("throttling"), "notify_one", stream));
        ...
        void unregister_polling(pika::threads::detail::thread_pool_base& pool)
        {
            ...
            if constexpr (mpi_debug<5>.is_enabled())
                PIKA_DP(mpi_debug<5>,
                    debug(debug::detail::str<>("disable polling")));
            auto* sched = pool.get_scheduler();
        ...
        bool init_mpi, std::string const& pool_name, bool init_errorhandler)
    {
        if (init_mpi)
        {
            ...
            if (provided < MPI_THREAD_FUNNELED)
            {
                PIKA_DP(detail::mpi_debug<5>,
                    error(debug::detail::str<>("pika::mpi::experimental::init"),
                        "init failed"));
        ...
        PIKA_DP(detail::mpi_debug<5>,
            debug(debug::detail::str<>("pika::mpi::experimental::init"),
                detail::mpi_data_));
    ...
    void finalize(std::string const& pool_name)
    {
        ...
        PIKA_DP(detail::mpi_debug<5>,
            debug(debug::detail::str<>("Clearing mode"), detail::mpi_data_,

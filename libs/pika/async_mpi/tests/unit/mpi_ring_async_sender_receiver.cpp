...
void msg_info(std::uint32_t rank, std::uint32_t size, msg_type mtype,
    message_item& token, unsigned tag, const char* xmsg = nullptr)
{
    if (output)
    {
        ...
        msr_deb<0>.debug(str<>(msg), temp.str(), "token",
            dec<3>(token.databuf[0]), "to/from", dec<3>(other), "tag",
            dec<3>(tag), xmsg);
    }
}
...
int pika_main(pika::program_options::variables_map& vm)
{
    ...
    // --------------------------
    {
        ...
        std::vector<message_item> tokens(
            safety_factor * in_flight * size, size);
        ...
        for (std::uint64_t i = 0; i < iterations; ++i)
        {
            ...
            for (int origin = 0; origin < size; origin++)
            {
                ...
                auto snd1 = ex::just(&tokens[tag], sizeof(message_item),
                                MPI_UNSIGNED_CHAR, prev_rank(rank, size), tag,
                                MPI_COMM_WORLD) |
                    ...
                    ex::then([=, &tokens, &counter](int /*res*/) {
                            ...
                            (size - std::uint64_t(rank - origin)) % size)
                        {
                            if (output)
                            {
                                ...
                                msr_deb<0>.debug(str<>("Recv"), "Rank",
                                    ...
                                    "Recv token",
                                    dec<3>(tokens[tag].databuf[0]), "from rank",
                                    dec<3>(prev_rank(rank, size)), "tag",
                                    dec<3>(tag));
                            }
                        }
                        assert(tokens[tag].databuf[0] ==
                            (size - std::uint64_t(rank - origin)) % size);
                        ...
                        else
                        {
                            ...
                            auto send_snd = as1 |
                                ...
                                ex::then([=, &tokens, &counter](int /*res*/) {
                                    // output info
                                    msg_info(rank, size, msg_type::send,
                                        tokens[tag], tag);
                                    counter--;
                                });
                            msg_info(rank, size, msg_type::send, tokens[tag],
                                tag, "post");
...
//                    next_rank(rank, size), tag, MPI_COMM_WORLD) |
        ...
        if (rank == 0)
        {
            ...
            char const* msg = "CSVData, "
                              ...
                              "iterations, {5}, task_transfer, {6} time";

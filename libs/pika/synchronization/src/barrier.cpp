...
// The implementation is based on the tree barrier from libcxx with the license below. See header
...
namespace pika::detail {
    ...
    bool barrier_algorithm_base::arrive(std::ptrdiff_t expected, detail::barrier_phase_t old_phase)
    {
        ...
        // The original libcxx implementation uses only the id of the current std::thread as the
        // input for the hash. This implementation uses the pika thread id if available, and
        // otherwise the std::thread id. This assumes that pika thread ids and std::thread ids
        // do not coincide.

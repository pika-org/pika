...
// The implementation is based on the tree barrier from libc++ with the license below. Compared to
// the original:
//
...
//   after some time)
//
// This implementation was last updated with
// - https://github.com/llvm/llvm-project/blob/dc57752031fb14166dff2174b36c28d27d742382/libcxx/include/barrier
// - https://github.com/llvm/llvm-project/blob/dc57752031fb14166dff2174b36c28d27d742382/libcxx/src/barrier.cpp
...
namespace pika {
    ...
    namespace detail {
        ...
        public:
            ...
            explicit barrier_algorithm_base(std::ptrdiff_t expected);
    ...
    public:
        ...
        //                 constructor.
        explicit barrier(std::ptrdiff_t expected, Completion completion = Completion())
          : expected(expected)
          ...
          , base(this->expected)
        {
            PIKA_ASSERT(expected >= 0);
            PIKA_ASSERT(expected <= (max) ());
        }
        ...
        {
            auto const poll = [&]() {
                // The original libc++ implementation uses the inverse condition here, since it

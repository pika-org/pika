...
namespace pika { namespace parallel { namespace execution {
    ...
    namespace detail {
        ...
        PIKA_EXPORT void intrusive_ptr_add_ref(shape_iter_impl_base* p);
        PIKA_EXPORT void intrusive_ptr_release(shape_iter_impl_base* p);
        ...
        struct never_blocking_oneway_vtable<R(Ts...)>
        {
            ...
            {
                execution::post(vtable_base::get<T>(exec), PIKA_MOVE(f),
                    PIKA_FORWARD(Ts, ts)...);
            }
        ...
        struct twoway_vtable<R(Ts...)>
        {
            ...
            static pika::future<R> _empty_async_execute(void* /* exec */,
                async_execute_function_type&& /* f */, Ts&&... /* ts */)
            {
                throw_bad_polymorphic_executor<R>();
                return {};
            }
            static pika::future<R> _empty_then_execute(void*,
                then_execute_function_type&&, pika::shared_future<void> const&,
                Ts&&...)
            {
                throw_bad_polymorphic_executor<R>();
                return {};
        ...
        struct bulk_twoway_vtable<R(Ts...)>
        {
            ...
            // bulk_async_execute
            template <typename T>
            static std::vector<pika::future<R>> _bulk_async_execute(void* exec,
            ...
            std::vector<pika::future<R>> (*bulk_async_execute)(void*,
                bulk_async_execute_function_type&&, range_proxy const& shape,
                Ts&&...);

            // bulk_then_execute
            template <typename T>
            static pika::future<std::vector<R>> _bulk_then_execute(void* exec,
                bulk_then_execute_function_type&& f, range_proxy const& shape,
                pika::shared_future<void> const& predecessor, Ts&&... ts)
            {
                return execution::bulk_then_execute(vtable_base::get<T>(exec),
                    PIKA_MOVE(f), shape, predecessor, PIKA_FORWARD(Ts, ts)...);
            }
            pika::future<std::vector<R>> (*bulk_then_execute)(void*,
            ...
            static std::vector<pika::future<R>> _empty_bulk_async_execute(void*,
            ...
            static pika::future<std::vector<R>> _empty_bulk_then_execute(void*,
        ...
        class PIKA_EXPORT polymorphic_executor_base
        {
    ...
    private:
        ...
        void assign(Exec&& exec)
        {
            ...
            if (!detail::is_empty_polymorphic_executor(exec))
            {
                ...
                object = ::new (buffer) T(PIKA_FORWARD(Exec, exec));
            }
            else
            {
                base_type::reset(get_empty_vtable());
    ...
    public:
        ...
        template <typename T>
        using future_type = pika::future<R>;

        // NonBlockingOneWayExecutor interface
        template <typename F>
        PIKA_FORCEINLINE void post(F&& f, Ts... ts) const
        {
            using function_type = typename vtable::post_function_type;

            vtable const* vptr = static_cast<vtable const*>(base_type::vptr);
            vptr->post(object, function_type(PIKA_FORWARD(F, f)),
                PIKA_FORWARD(Ts, ts)...);
        }

        // OneWayExecutor interface
        template <typename F>
        PIKA_FORCEINLINE R sync_execute(F&& f, Ts... ts) const
        {
            using function_type = typename vtable::sync_execute_function_type;

            vtable const* vptr = static_cast<vtable const*>(base_type::vptr);
            return vptr->sync_execute(object, function_type(PIKA_FORWARD(F, f)),
                PIKA_FORWARD(Ts, ts)...);
        }

        // TwoWayExecutor interface
        template <typename F>
        PIKA_FORCEINLINE pika::future<R> async_execute(F&& f, Ts... ts) const
        {
            ...
            F&& f, Future&& predecessor, Ts&&... ts) const
        {
            ...
            vtable const* vptr = static_cast<vtable const*>(base_type::vptr);
            return vptr->then_execute(object, function_type(PIKA_FORWARD(F, f)),
            ...
            F&& f, Shape const& s, Ts&&... ts) const
        {
            ...
            return vptr->bulk_sync_execute(object,
                function_type(PIKA_FORWARD(F, f)), shape,
                PIKA_FORWARD(Ts, ts)...);
        }
            ...
            Ts&&... ts) const
        {
            ...
            return vptr->bulk_then_execute(object,
                function_type(PIKA_FORWARD(F, f)), shape, predecessor,
                PIKA_FORWARD(Ts, ts)...);
        }

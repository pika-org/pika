...
using pika::program_options::detail::cmdline;
    ...
    const test_case* cases)
{
    for (int i = 0; cases[i].input; ++i)
    {
        ...
        PIKA_TEST_EQ(status, cases[i].expected_status);
        PIKA_TEST_EQ(result, cases[i].expected_result);
    }
}
...
void test_additional_parser()
{
    ...
    // the long_names() API function was introduced in Boost V1.68
    PIKA_TEST_EQ(result.size(), std::size_t(3));
    PIKA_TEST_EQ(result[0].string_key, "response-file");
    PIKA_TEST_EQ(result[0].value[0], "config");
    PIKA_TEST_EQ(result[1].string_key, "foo");
    PIKA_TEST_EQ(result[1].value[0], "1");
    // the long_names() API function was introduced in Boost V1.68
    PIKA_TEST_EQ(result[2].string_key, "bar");
    PIKA_TEST_EQ(result[2].value[0], "11");
...
void test_style_parser()
{
    ...
    PIKA_TEST_EQ(result.size(), std::size_t(2));
    PIKA_TEST_EQ(result[0].string_key, "foo");
    PIKA_TEST_EQ(result[0].value[0], "1");
    PIKA_TEST_EQ(result[1].string_key, "bar");
    PIKA_TEST_EQ(result[1].value[0], "1");
}
...
void test_unregistered()
{
    ...
    vector<option> result = cmd.run();
    PIKA_TEST_EQ(result.size(), std::size_t(5));
    // --foo=1
    PIKA_TEST_EQ(result[0].string_key, "foo");
    PIKA_TEST_EQ(result[0].unregistered, true);
    PIKA_TEST_EQ(result[0].value[0], "1");
    // --bar
    PIKA_TEST_EQ(result[1].string_key, "bar");
    PIKA_TEST_EQ(result[1].unregistered, true);
    PIKA_TEST(result[1].value.empty());
    // '1' is considered a positional option, not a value to
    // --bar
    PIKA_TEST(result[2].string_key.empty());
    PIKA_TEST_EQ(result[2].position_key, 0);
    PIKA_TEST_EQ(result[2].unregistered, false);
    PIKA_TEST_EQ(result[2].value[0], "1");
    // -b
    PIKA_TEST_EQ(result[3].string_key, "-b");
    PIKA_TEST_EQ(result[3].unregistered, true);
    PIKA_TEST(result[3].value.empty());
    // -biz
    PIKA_TEST_EQ(result[4].string_key, "-b");
    PIKA_TEST_EQ(result[4].unregistered, true);
    PIKA_TEST_EQ(result[4].value[0], "iz");
    ...
    PIKA_TEST_EQ(result.size(), std::size_t(3));
    PIKA_TEST_EQ(result[0].string_key, "help");
    PIKA_TEST_EQ(result[0].unregistered, false);
    PIKA_TEST(result[0].value.empty());
    PIKA_TEST_EQ(result[1].string_key, "-c");
    PIKA_TEST_EQ(result[1].unregistered, true);
    PIKA_TEST(result[1].value.empty());
    PIKA_TEST_EQ(result[2].string_key, "magic");
    PIKA_TEST_EQ(result[2].unregistered, false);
    PIKA_TEST_EQ(result[2].value[0], "c");

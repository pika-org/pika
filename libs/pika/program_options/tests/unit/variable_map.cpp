...
void test_variable_map()
{
    ...
    store(a3, vm);
    notify(vm);
    PIKA_TEST_EQ(vm.size(), std::size_t(4));
    PIKA_TEST_EQ(vm["foo"].as<string>(), "'12'");
    PIKA_TEST_EQ(vm["bar"].as<string>(), "11");
    PIKA_TEST_EQ(vm.count("biz"), std::size_t(1));
    PIKA_TEST_EQ(vm["biz"].as<string>(), "3");
    PIKA_TEST_EQ(vm["output"].as<string>(), "foo");
    ...
    store(a4, vm2);
    notify(vm2);
    PIKA_TEST_EQ(vm2.size(), std::size_t(3));
    PIKA_TEST_EQ(vm2["zee"].as<bool>(), true);
    PIKA_TEST_EQ(vm2["zak"].as<int>(), 13);
    PIKA_TEST_EQ(vm2["opt"].as<bool>(), false);
    PIKA_TEST_EQ(i, 13);
    ...
    store(a5, vm3);
    notify(vm3);
    PIKA_TEST_EQ(vm3.size(), std::size_t(3));
    PIKA_TEST_EQ(vm3["vee"].as<string>(), "42");
    PIKA_TEST_EQ(vm3["voo"].as<string>(), "1");
    PIKA_TEST_EQ(vm3["iii"].as<int>(), 123);
    ...
    store(a6, vm4);
    notify(vm4);
    PIKA_TEST_EQ(vm4.size(), std::size_t(4));
    PIKA_TEST_EQ(vm4["imp"].as<int>(), 1);
    PIKA_TEST_EQ(vm4["iim"].as<int>(), 201);
    PIKA_TEST_EQ(vm4["mmp"].as<int>(), 123);
}
...
void test_semantic_values()
{
    ...
    string exp2[] = {"q", "q", "w"};
    PIKA_TEST(av2 == vector<string>(exp2, exp2 + 3));
    ...
    store(parsed, vm2);
    notify(vm2);
    PIKA_TEST_EQ(vm2.count("int"), std::size_t(1));
    PIKA_TEST(vm2["int"].as<vector<int>>() == vector<int>(1, 13));
    PIKA_TEST_EQ(stored_value, 13);

    vector<option> saved_options = options;

    options.emplace_back(option("bar", vector<string>(1, "2")));
    variables_map vm3;
    PIKA_TEST_THROW(store(parsed, vm3), multiple_occurrences);
    ...
    variables_map vm4;
    PIKA_TEST_THROW(store(parsed, vm4), validation_error);
}
...
void test_priority()
{
    ...
    PIKA_TEST_EQ(vm.count("first"), std::size_t(1));
    PIKA_TEST_EQ(vm["first"].as<vector<int>>().size(), std::size_t(2));
    PIKA_TEST_EQ(vm["first"].as<vector<int>>()[0], 1);
    PIKA_TEST_EQ(vm["first"].as<vector<int>>()[1], 3);

    PIKA_TEST_EQ(vm.count("second"), std::size_t(1));
    PIKA_TEST_EQ(vm["second"].as<vector<int>>().size(), std::size_t(1));
    PIKA_TEST_EQ(vm["second"].as<vector<int>>()[0], 1);
    ...
    PIKA_TEST_EQ(vm.count("second"), std::size_t(1));
    PIKA_TEST_EQ(vm["second"].as<vector<int>>().size(), std::size_t(1));
    PIKA_TEST_EQ(vm["second"].as<vector<int>>()[0], 7);

    PIKA_TEST_EQ(vm.count("include"), std::size_t(1));
    PIKA_TEST_EQ(vm["include"].as<vector<int>>().size(), std::size_t(2));
    PIKA_TEST_EQ(vm["include"].as<vector<int>>()[0], 1);
    PIKA_TEST_EQ(vm["include"].as<vector<int>>()[1], 7);
}
...
void test_multiple_assignments_with_different_option_description()
{
    ...
    PIKA_TEST_EQ(vm.count("help"), std::size_t(1));
    PIKA_TEST_EQ(vm.count("includes"), std::size_t(1));
    PIKA_TEST_EQ(vm["includes"].as<vector<string>>()[0], "a");
    PIKA_TEST_EQ(vm["includes"].as<vector<string>>()[1], "b");
}

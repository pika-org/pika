...
////////////////////////////////////////////////////////////////////////////////
#if defined(PIKA_MSVC) && PIKA_MSVC >= 1400
#pragma warning(push)
...
}}}}    // namespace pika::util::detail::any
...
namespace pika { namespace util {
    ...
    private:
        ...
        {
            using value_type = typename std::decay<T>::type;
            new (&object) value_type(PIKA_FORWARD(Ts, ts)...);
        }
        ...
        {
            using value_type = typename std::decay<T>::type;
            object = new value_type(PIKA_FORWARD(Ts, ts)...);
        }
    ...
    public:
        ...
        {
            basic_any(PIKA_FORWARD(T, rhs)).swap(*this);
            return *this;
        }
        ...
        {
            if (type() != typeid(T))
                throw pika::bad_any_cast(type(), typeid(T));

            return pika::util::detail::any::get_table<T>::is_small::value ?
    ...
    ////////////////////////////////////////////////////////////////////////////
    // specialization for pika::any supporting streaming
...
namespace pika {
    ...
    T any_cast(util::basic_any<IArch, OArch, Char, Copyable>& operand)
    {
        ...
        nonref* result = pika::any_cast<nonref>(&operand);
        if (!result)
            throw pika::bad_any_cast(operand.type(), typeid(T));
        ...
        util::basic_any<IArch, OArch, Char, Copyable> const& operand)
    {
        ...
        return pika::any_cast<nonref const&>(
            const_cast<util::basic_any<IArch, OArch, Char, Copyable>&>(
                operand));
    }
}    // namespace pika
...
namespace pika { namespace util {
    ...
    basic_any<void, void, Char, std::true_type> make_streamable_any_nonser(
        T&& t)
    {
        return basic_any<void, void, Char, std::true_type>(PIKA_FORWARD(T, t));
    }

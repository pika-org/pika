...
namespace pika { namespace util {
    ...
    public:
          ...
          : empty_(true)
        {
            if (!other.empty_)
            {
                new (&storage_) T(PIKA_MOVE(other.value()));
                empty_ = false;
            }
        }
        ...
        explicit optional(in_place_t, Ts&&... ts)
          : empty_(true)
        {
            new (&storage_) T(PIKA_FORWARD(Ts, ts)...);
            empty_ = false;
        }
        ...
        explicit optional(in_place_t, std::initializer_list<U> il, Ts&&... ts)
          : empty_(true)
        {
            new (&storage_) T(il, PIKA_FORWARD(Ts, ts)...);
            empty_ = false;
        }
        ...
        optional& operator=(T&& other)
        {
            ...
            new (&storage_) T(PIKA_MOVE(other));
            empty_ = false;
        ...
        void emplace_f(F&& f, Ts&&... ts)
        {
            ...
            new (&storage_) T(PIKA_FORWARD(F, f)(PIKA_FORWARD(Ts, ts)...));
            empty_ = false;
        }
#endif
                ...
                _optional_swap::check_swap<T>()))
        {
            ...
            new (&empty->storage_) T(PIKA_MOVE(**non_empty));
    ...
    {
        return optional<typename std::decay<T>::type>(PIKA_FORWARD(T, v));
    }

    template <typename T, typename... Ts>
    constexpr optional<T> make_optional(Ts&&... ts)
    {
        return optional<T>(in_place, PIKA_FORWARD(Ts, ts)...);
    }

    template <typename T, typename U, typename... Ts>
    constexpr optional<T> make_optional(std::initializer_list<U> il, Ts&&... ts)
    {
        return optional<T>(in_place, il, PIKA_FORWARD(Ts, ts)...);
    }
}}    // namespace pika::util
...
namespace std {
    template <typename T>
    struct hash<pika::optional<T>>
    {
        constexpr std::size_t operator()(::pika::optional<T> const& arg) const
        {

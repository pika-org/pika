...
#pragma clang diagnostic pop
    ...
    const pika::util::function_ref<void()> v7(one_or_two);
    pika::util::function_ref<void()> v8(one_or_two);
...
static void test_one_arg()
{
    ...
    pika::util::function_ref<int(int)> f1(neg);
    PIKA_TEST_EQ(f1(5), -5);

    pika::util::function_ref<string(string)> id(&identity_str);
    PIKA_TEST_EQ(id("str"), "str");

    pika::util::function_ref<string(const char*)> id2(&identity_str);
    PIKA_TEST_EQ(id2("foo"), "foo");

    add_to_obj add_to(5);
    pika::util::function_ref<int(int)> f2(add_to);
    PIKA_TEST_EQ(f2(3), 8);

    const pika::util::function_ref<int(int)> cf2(add_to);
    PIKA_TEST_EQ(cf2(3), 8);
}

static void test_two_args()
{
    pika::util::function_ref<string(const string&, const string&)> cat(
        &string_cat);
    PIKA_TEST_EQ(cat("str", "ing"), "string");

    pika::util::function_ref<int(short, short)> sum(&sum_ints);
...
static void test_ref()
{
    ...
    try
    {
        pika::util::function_ref<int(int, int)> f(std::ref(atc));

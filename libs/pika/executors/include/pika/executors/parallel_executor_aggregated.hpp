...
namespace pika { namespace parallel { namespace execution {
    ...
    template <typename Policy = pika::launch::async_policy>
    struct parallel_policy_executor_aggregated
    {
        ...
        template <typename F, typename... Ts>
        static pika::future<void> async_execute(F&& f, Ts&&... ts)
        {
            return pika::detail::async_launch_policy_dispatch<Policy>::call(
                Policy{}, PIKA_FORWARD(F, f), PIKA_FORWARD(Ts, ts)...);
        }
        ...
        static void post(F&& f, Ts&&... ts)
        {
            ...
            detail::post_policy_dispatch<Policy>::call(
                Policy{}, desc, PIKA_FORWARD(F, f), PIKA_FORWARD(Ts, ts)...);
        }
            ...
            void operator()(F&& f, S const& shape, Ts&&... ts) const
            {
                ...
                if (pika::detail::has_async_policy(Policy{}))
                {
                    spawn_hierarchical(l, size, num_tasks, f,
                        pika::util::begin(shape), e, mtx_e, ts...);
                }
                else
                {
                    spawn_sequential(
                        l, size, f, pika::util::begin(shape), e, mtx_e, ts...);
                }
                l.wait();
                ...
                if (e)
                {
                    std::rethrow_exception(PIKA_MOVE(e));
                }
            }
                ...
                lcos::local::spinlock& mtx_e, Ts&&... ts) const
            {
                if (size > num_tasks)
                {
                    ...
                    while (size > chunk_size)
                    {
                        ...
                        it = pika::parallel::v1::detail::next(it, chunk_size);
                        size -= chunk_size;
                    }
                }
    ...
    public:
            ...
            F&& f, S const& shape, Ts&&... ts) const
        {
            ...
            std::vector<pika::future<void>> result;
    ...
    ///////////////////////////////////////////////////////////////////////////
    template <>
    struct parallel_policy_executor_aggregated<pika::launch>
    {
        /// Associate the parallel_execution_tag executor tag type as a default
        /// with this executor.
        using execution_category = pika::execution::parallel_execution_tag;
        ...
        void post(F&& f, Ts&&... ts) const
        {
            ...
            detail::post_policy_dispatch<pika::launch>::call(
                policy_, desc, PIKA_FORWARD(F, f), PIKA_FORWARD(Ts, ts)...);
        }
    ...
    private:
        ...
        struct sync_exec
        {
            ...
            void operator()(F&& f, S const& shape, Ts&&... ts) const
            {
                ...
                std::size_t size = pika::util::size(shape);
                lcos::local::latch l(size);
                if (pika::detail::has_async_policy(policy_))
                {
                    spawn_hierarchical(l, size, num_tasks, f,
                        pika::util::begin(shape), e, mtx_e, ts...);
                }
    ...
    using parallel_executor_aggregated =
        parallel_policy_executor_aggregated<pika::launch::async_policy>;
